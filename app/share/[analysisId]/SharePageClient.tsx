...
type LevelEstimate = { label: string; detail: string };

const clamp = (v: number, min: number, max: number) => Math.min(max, Math.max(min, v));

function estimateLevel(scoreRaw: number): LevelEstimate {
...
function computeRoundFallback(totalScore: number) {
...
export default function SharePageClient(props: {
  ...
  const levelEstimate = useMemo(() => estimateLevel(score), [score]);
  const round = useMemo(() => computeRoundFallback(score), [score]);
