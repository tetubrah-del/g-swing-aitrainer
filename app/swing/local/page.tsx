...
import { extractClientPhaseFrames } from "@/app/lib/client/extractClientPhaseFrames";
import { ClientPhaseFrame } from "@/app/lib/client/swingPhases";

export default function LocalSwingPage() {
  const [file, setFile] = useState<File | null>(null);
  const [frames, setFrames] = useState<ClientPhaseFrame[] | null>(null);
  ...
  const handleExtract = useCallback(async () => {
    ...
    setFrames(null);
    try {
      const result = await extractClientPhaseFrames({ file, maxWidth: 480 });
      setFrames(result);
    } catch (err) {
      ...
      {frames && (
        <div className="flex flex-col gap-4">
            ...
            {frames.map((frame) => (
              ...
              >
                <div className="mb-2 flex items-center justify-between text-sm font-medium">
                  <span className="capitalize">{frame.phase}</span>
                  <span className="text-gray-500">{frame.timestampSec.toFixed(2)}s</span>
                </div>
                {/* Base64 デバッグ用途のため通常の img を使用 */}
          ...
          <div>
            ...
            <pre className="whitespace-pre-wrap rounded bg-gray-100 p-3 text-xs">{JSON.stringify(frames, null, 2)}</pre>
          </div>
        </div>
      )}
    </div>
