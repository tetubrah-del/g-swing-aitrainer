...
export function genPrompt(meta?: GolfAnalyzeMeta, previousReport?: SwingAnalysis | null): string {
  ...
  const systemPrompt = [
    ...
    "  - アウトサイドインを「確定」できる場合のみ、issues に必ず「アウトサイドイン（確定）」を含め、major_ng.downswing = true かつ phases.downswing.score <= 8 にする",
    "  - 断定できないが外から入りやすい場合は、issues に必ず「外から入りやすい傾向」を含め、mid_high_ok.downswing = false かつ phases.downswing.score は 9〜12 にする（確定ワードは書かない）",
