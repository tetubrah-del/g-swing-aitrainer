  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const res = await fetch(`/api/golf/history?anonymousUserId=${encodeURIComponent(anonymousId)}`);
      ...
      <div className="w-full max-w-4xl px-4 py-8 space-y-8">
        ...
        {isAnonymous && (
          <section className="rounded-xl bg-slate-900/70 border border-slate-800 p-4 space-y-3">
            ...
            <button className="rounded-md bg-emerald-500 hover:bg-emerald-400 px-4 py-2 text-sm font-semibold text-slate-900">
              メールアドレスを登録する
            </button>
          </section>
        )}
